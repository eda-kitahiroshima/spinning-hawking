...
app.post('/api/apps', upload.single('screenshot'), async (req, res) => {
  const { name, description, code, tags, userId } = req.body;
  const getPublicUrl = (file) => {
    if (!file) return null;
    if (process.env.AWS_PUBLIC_DOMAIN && file.key) {
      return `${process.env.AWS_PUBLIC_DOMAIN}/${file.key}`;
  ...
  try {
    ...
    // Auto-generate URL
    const autoGeneratedUrl = `http://localhost:5000/preview/${appId}`;
    await db.query("UPDATE apps SET downloadUrl = $1 WHERE id = $2", [autoGeneratedUrl, appId]);

    console.log("✅ App Created!");
    console.log("📷 Screenshot URL:", screenshotUrl);

    res.status(201).json({ id: appId, downloadUrl: autoGeneratedUrl, message: "Success" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
